...
- Revision practice parity (single-example rotation + Classic DOM/classes)
...
7. Revision practice: one example at a time, Next cycles, apply/dismiss removes from rotation.
8. Confirm `power_verbs_2025.json` loads once (no repeated requests).
9. Ensure hardening flags are on → cancel, toasts, validation, timeouts.
10. In Diagnostics → “Copy debug info” and confirm secrets are redacted.
