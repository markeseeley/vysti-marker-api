...
def is_embedded_device(thesis_tokens, i):
    ...
    while j >= 0:
        ...
        # Hard boundaries: if we hit punctuation or 'and', we are
        # starting a new list item / phrase.
        if txt in {",", ";"} or lower == "and":
            break
    ...
    for s_start, s_end in raw_sentences:
        ...
        while i < s_end - 1:
            ...
            if ch in ".?!" and not nxt.isspace() and nxt.isalpha() and nxt.isupper():
                cut_end = i + 1  # sentence ends just after the punctuation
...
def add_summary_table(doc, labels, rules):
    """
...
def run_marker(
    ...
    add_summary_table(doc, labels_used, rules)
